...
def test_3pl_recovery(replication):
    truth, draws, divergences = fit_simulated("3PL", replication)
    ...
    # Broad regression check: asymptotes are weakly informed in these small fits.
    assert rmse(draws["lower_asymptote"], truth["lower_asymptote"]) < 0.15
    assert ((draws["lower_asymptote"] > 0) & (draws["lower_asymptote"] < 1)).all()
